...
import { 
  ...
  Calendar, Upload, CheckCircle2, GraduationCap 
} from "lucide-react";
...
const TrainingRegistration = ({ training_package, selected_id }) => {
  ...
  const currentPackage = training_package.find(pkg => pkg.id == data.package_id);

  // Validation helper
  const validateForm = () => {
    if (!data.package_id || !data.name || !data.email || !data.phone || !data.address || !data.nid || !data.age) {
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-gray-500 mt-2">Provide your details to join the academy</p>
        </div>
        ...
        {currentPackage && (
          <div className="mb-8 bg-blue-50 border-2 border-[#1C398E] rounded-2xl p-6 flex flex-col md:flex-row justify-between items-center shadow-sm animate-in fade-in duration-500">
            ...
            <div className="text-right mt-4 md:mt-0">
              <div className="text-3xl font-black text-[#1C398E]">${currentPackage.price}</div>
              <div className="text-sm font-medium text-gray-500">{currentPackage.duration}</div>
            </div>
          </div>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2 space-y-2">
                <label className="text-sm font-bold text-gray-700 flex items-center gap-2">
                  <CheckCircle2 size={16} className="text-[#1C398E]" /> Select Training Package *
                </label>
                <select
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-gray-700 flex items-center gap-2">
                  <User size={16} className="text-[#1C398E]" /> Full Name *
                </label>
                <input
                  type="text" 
                  value={data.name} 
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-gray-700 flex items-center gap-2">
                  <Mail size={16} className="text-[#1C398E]" /> Email Address *
                </label>
                <input
                  type="email" 
                  value={data.email} 
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-gray-700 flex items-center gap-2">
                  <Phone size={16} className="text-[#1C398E]" /> Phone Number *
                </label>
                <input
                  type="tel" 
                  value={data.phone} 
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-gray-700 flex items-center gap-2">
                  <GraduationCap size={16} className="text-[#1C398E]" /> Educational Qualification *
                </label>
                <input
                  type="text" 
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-gray-700 flex items-center gap-2">
                  <Calendar size={16} className="text-[#1C398E]" /> Age *
                </label>
                <input
                  type="number" 
                  value={data.age} 
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-gray-700 flex items-center gap-2">
                  <CreditCard size={16} className="text-[#1C398E]" /> NID / Birth Certificate *
                </label>
                <input
                  type="text" 
                  value={data.nid} 
              ...
              <div className="md:col-span-2 space-y-2">
                <label className="text-sm font-bold text-gray-700 flex items-center gap-2">
                  <MapPin size={16} className="text-[#1C398E]" /> Permanent Address *
                </label>
                <input
                  type="text" 
                  value={data.address} 
              ...
              <div className="md:col-span-2 space-y-2">
                <label className="text-sm font-bold text-gray-700 flex items-center gap-2">
                  <Upload size={16} className="text-[#1C398E]" /> Profile Image *
                </label>
                <div className="relative group border-2 border-dashed border-gray-200 rounded-xl p-6 hover:border-[#1C398E] transition text-center bg-gray-50">
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-gray-500">
                      {data.image ? (
                        <span className="text-[#1C398E] font-bold">{data.image.name}</span>
                      ) : (
