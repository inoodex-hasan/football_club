    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="relative h-64 md:h-96 overflow-hidden">
        ...
        <div className="container absolute bottom-0 left-0 right-0 p-8 md:p-16 text-white mx-auto">
          <h1 className="text-4xl md:text-6xl font-bold mb-4">Our Media Galleries</h1>
          <p className="text-lg md:text-xl max-w-3xl opacity-90">Capturing every moment on and off the field.</p>
        </div>
      </section>
      ...
      <section className="py-16 px-4 md:px-12">
        <div className="container mx-auto">
          <h2 className="text-3xl md:text-5xl font-bold text-center mb-12 text-[#283E77]">
            Media Gallery
          </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {mediaItems.map((item, index) => (
              ...
              >
                {item.type === "image" ? (
                  <img src={item.src} alt={item.title} className="w-full h-64 object-cover transform group-hover:scale-105 transition duration-500" />
                ) : (
                  <div className="relative w-full h-64 bg-black">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="bg-white/90 rounded-full p-3 shadow-lg group-hover:scale-110 transition">
                        <svg className="w-10 h-10 text-[#283E77]" fill="currentColor" viewBox="0 0 20 20">
      ...
      {/* Lightbox Modal */}
      {lightboxOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/95 animate-in fade-in duration-300" onClick={closeLightbox}>
          <button onClick={closeLightbox} className="absolute top-6 right-8 text-white text-5xl font-light hover:text-gray-400 z-[10000]">&times;</button>
          
          <button onClick={goPrev} className="absolute left-6 text-white text-6xl font-thin hover:text-gray-400 transition z-[10000]">&#8249;</button>
          <button onClick={goNext} className="absolute right-6 text-white text-6xl font-thin hover:text-gray-400 transition z-[10000]">&#8250;</button>

          <div className="max-w-6xl w-full mx-auto p-4 flex flex-col items-center" onClick={(e) => e.stopPropagation()}>
            {mediaItems[currentMediaIndex].type === "image" ? (
              <img src={mediaItems[currentMediaIndex].src} className="max-h-[80vh] w-auto object-contain rounded-lg shadow-2xl" />
            ) : (
              <video src={mediaItems[currentMediaIndex].src} controls autoPlay className="max-h-[80vh] w-full rounded-lg shadow-2xl">
